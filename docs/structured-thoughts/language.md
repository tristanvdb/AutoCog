...
Free-form text generation. The model produces exactly `length` tokens.

```
name is text;                    // unbounded
name is text<length=50>;         // exactly 50 tokens
name is text<length=50, threshold=0.9>;  // stop early if confident
```
...
Clauses modify how call results are integrated:

| Clause | Description |
|--------|-------------|
| `bind(a, b)` | Extract named fields from the result |
| `ravel` | Flatten array results into the target array |
| `mapped` | Call once per element of the source array |
| `wrap` | Wrap scalar result into a single-element array |
| `prune` | Remove empty/null entries |
...
```
return {
  use answer;                       // return a single field
}

return {
  status is "complete";             // literal
